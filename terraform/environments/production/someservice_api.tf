...
resource "google_dns_record_set" "someservice_api" {
  name = "someservice-api.${google_dns_managed_zone.getsomeservice.dns_name}"
  type = "A"
  ttl  = 300
...
variable "getsomeservice_web_cname" {
  description = "Type CNAME DNS Record value of app.getsomeservice.com"
  type        = string
}

resource "google_dns_record_set" "someservice_webservice_cname" {
  name = "app.${google_dns_managed_zone.getsomeservice.dns_name}"
  type = "CNAME"
  ttl  = 300

  managed_zone = google_dns_managed_zone.getsomeservice.name

  rrdatas = [var.getsomeservice_web_cname]
}
...
resource "google_sql_database_instance" "someservice_api" {
  name             = "someservice-someservice-api"
...
variable "someservice_api_db_credential" {
  description = "user/password of someservice_api_db"
  type = object({
    db_user     = string
    db_password = string
  })
